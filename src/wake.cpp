...
void wake_on_bt_input(const uint8_t *hid_input, uint16_t len) {
    if (!get_config().enable_wake) return;
    if (len < 10) return;
    // DS4 BT 0x11 input report layout (after main.cpp's `data + 4` skip):
    //   byte 4 low nibble: D-pad direction (0x08 idle); high nibble: face buttons
    //   byte 5: L1, R1, L2 click, R2 click, share, options, L3, R3
    //   byte 6: PS (bit 0), touchpad-click (bit 1); bits 2-7 are a report
    //           counter and must be masked, or every report looks like a press
    //
    ...
    const uint8_t b7 = hid_input[4];
    const uint8_t b8 = hid_input[5];
    const uint8_t b9 = hid_input[6] & 0x03;
