...
void battery_led_on_disconnect(void) {
    ...
    // 0x11 report arrives on the next connection — prevents the cached
...
void battery_led_tick(void) {
    ...
    // DS4 battery/ext byte: bits 0-3 level (0-10 on battery), bit 4 cable.
    const uint8_t b   = interrupt_in_data[29];
    const uint8_t pct = b & 0x0F;
    const bool discharging = (b & 0x10) == 0;
    const bool low    = discharging && (pct <= THRESHOLD_LEVEL);
